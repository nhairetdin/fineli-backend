...
ALTER TABLE base ADD PRIMARY KEY(foodid);
ALTER TABLE base ADD CONSTRAINT fk_base_elintarvike1 FOREIGN KEY (foodid) REFERENCES elintarvike(foodid);
CREATE UNIQUE INDEX idx_base_foodid1 ON base(foodid);

CREATE TABLE ravintotekija_all
  SELECT 
    ravintotekija.eufdname, 
    ravintotekija.compunit, 
    ravintotekija.cmpclass, 
    ravintotekija.cmpclassp, 
    ravintotekijanimi.thscode AS namecode, 
    ravintotekijanimi.description AS namedescription, 
    ravintotekijanimi.lang AS namelang,
    ravintotekijayksikko.thscode AS unitcode,
    ravintotekijayksikko.description AS unitdescription,
    ravintotekijayksikko.lang AS unitlang,
    ravintotekijaluokka.thscode AS classcode,
    ravintotekijaluokka.description AS classdescription,
    ravintotekijaluokka.lang AS classlang
  FROM ravintotekija 
    JOIN ravintotekijanimi 
      ON ravintotekija.eufdname = ravintotekijanimi.thscode 
    JOIN ravintotekijayksikko 
      ON ravintotekija.compunit = ravintotekijayksikko.thscode 
    JOIN ravintotekijaluokka 
      ON ravintotekija.cmpclass = ravintotekijaluokka.thscode;

CREATE INDEX idx_ravintotekijaall_1 ON ravintotekija_all(eufdname);
...
ALTER TABLE ravintoarvo ADD CONSTRAINT fk_ravintoarvo_6 FOREIGN KEY (foodid) REFERENCES base(foodid);
ALTER TABLE ravintoarvo ADD CONSTRAINT fk_ravintoarvo_5 FOREIGN KEY (eufdname) REFERENCES ravintotekija_all(eufdname);
ALTER TABLE ateria_elintarvike ADD CONSTRAINT fk_ateria_elintarvike_3 FOREIGN KEY (foodid) REFERENCES base(foodid);
ALTER TABLE resepti ADD CONSTRAINT fk_resepti_3 FOREIGN KEY (foodid) REFERENCES base(foodid);
ALTER TABLE elintarvikemitat ADD CONSTRAINT fk_elintarvikemitat_3 FOREIGN KEY (foodid) REFERENCES base(foodid);
